...
pub fn fingerprint_did_key_material(
    material: &acdp_did::key::DidKeyMaterial,
) -> Result<String, AcdpError> {
    match material {
        acdp_did::key::DidKeyMaterial::Ed25519(pk) => Ok(fingerprint_ed25519(pk)),
        acdp_did::key::DidKeyMaterial::EcdsaP256(sec1) => fingerprint_p256_sec1(sec1),
    }
}
...
pub fn fingerprint_verification_method(
    method: &acdp_did::document::VerificationMethod,
    algorithm: &str,
) -> Result<String, AcdpError> {
    match algorithm {
...
pub async fn fingerprint_for_key_id(
    key_id: &str,
    algorithm: &str,
    resolver: &acdp_did::web::WebResolver,
) -> Result<String, AcdpError> {
    ...
    if did_part.starts_with("did:key:") {
        ...
        // refuses.
        let material = acdp_did::key::resolve_did_key_url(key_id)?;
...
mod tests {
    ...
    fn fp_001_ed25519_zero_seed() {
        let key = crate::SigningKey::from_bytes(&[0u8; 32]);
    ...
    fn did_key_material_matches_raw() {
        let key = crate::SigningKey::from_bytes(&[3u8; 32]);
        let did = acdp_did::key::did_key_from_ed25519(&key.verifying_key_bytes());
        let material = acdp_did::key::resolve_did_key(&did).unwrap();
        assert_eq!(
